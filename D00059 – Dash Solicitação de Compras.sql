...
SELECT DISTINCT 
	CASE WHEN COT.CODUSUREQ IS NULL THEN 0 ELSE COT.CODUSUREQ END value,
	CASE WHEN COT.CODUSUREQ IS NULL THEN 0 ELSE COT.CODUSUREQ END||' - '||
	CASE WHEN USU.NOMEUSU IS NULL THEN 'VAZIO' ELSE USU.NOMEUSU END label
FROM TGFCAB CAB 
...
--	AND GRU.CODGRUPOPROD = :A_GRUPO
	AND (CAB.CODVEND = (SELECT USU.CODVEND FROM TSIUSU USU WHERE USU.CODUSU = STP_GET_CODUSULOGADO) OR STP_GET_CODUSULOGADO IN (0,5,6,9,45,85,8,12,4,80,85))
   AND (CAB.NUMCOTACAO NOT IN (SELECT NUMCOTACAO FROM TGFITC WHERE NUNOTACPA IS NOT NULL) OR CAB.NUMCOTACAO IS NULL)
ORDER BY CAB.NUNOTA 
