...
SELECT * FROM TPRIPROC WHERE STATUSPROC = 'A'
...
--SUM((ITE.QTDNEG-ITE.QTDENTREGUE))QTDPENDENTE,
--(CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END) QTDESTOQUE,
--((CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END)-SUM((ITE.QTDNEG-ITE.QTDENTREGUE)))SALDO,
--CASE WHEN ((CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END)-SUM((ITE.QTDNEG-ITE.QTDENTREGUE)))> 0 THEN (((CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END)-SUM((ITE.QTDNEG-ITE.QTDENTREGUE))))*ICP.QTDMISTURA ELSE 0 END SALDO,
...
EST2.ESTOQUE, 
EST2.ESTOQUE - (CASE WHEN ((CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END)-SUM((ITE.QTDNEG-ITE.QTDENTREGUE)))<= 0 THEN (((CASE WHEN EST.ESTOQUE IS NULL THEN 0 ELSE EST.ESTOQUE END)-SUM((ITE.QTDNEG-ITE.QTDENTREGUE))))*ICP.QTDMISTURA ELSE 0 END)SALDO
FROM TGFCAB CAB
INNER JOIN tgfpar par ON cab.CODPARC = par.CODPARC 
INNER JOIN tgfite ite ON cab.nunota = ite.nunota
INNER JOIN tgfpro pro ON ite.codprod = pro.codprod
...
AND CAB.PENDENTE = 'S'
AND CAB.STATUSNOTA = 'L' --SIGNIFICA CONFIRMADA = SIM
AND ITE.CODPROD = 348 --:A_CODPROD1 
GROUP BY
ICP.CODMATPRIMA,
PRO1.DESCRPROD,
ICP.QTDMISTURA,
...
PC_PEND AS (SELECT DISTINCT ITE.CODPROD,PRO.DESCRPROD,SUM(ITE.QTDNEG)QTDNEG FROM TGFCAB CAB
INNER JOIN TGFITE ITE ON CAB.NUNOTA = ITE.NUNOTA
INNER JOIN TGFPRO PRO ON ITE.CODPROD = PRO.CODPROD
...
(CASE WHEN PC_PEND.QTDNEG IS NULL THEN 0 ELSE PC_PEND.QTDNEG END) PC_PENDENTE 
FROM MP1
INNER JOIN EST3 ON MP1.CODMATPRIMA = EST3.CODPROD
LEFT JOIN PC_PEND ON MP1.CODMATPRIMA = PC_PEND.CODPROD
GROUP BY MP1.CODMATPRIMA, MP1.DESCRPROD, EST3.ESTOQUE,PC_PEND.QTDNEG
ORDER BY 1
