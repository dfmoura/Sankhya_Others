...
WHERE ITE.CODPROD = 53


SELECT NUFIN, NUFIN - LAG(NUFIN) OVER (ORDER BY NUFIN) AS A FROM TGFFIN  
...
AD_TIPOCOBRANCA
 ('Serasa','Renegocia','Judicial','Simples')
Descontada


SELECT *
FROM TGFFIN
WHERE AD_TIPOCOBRANCA != 'Descontada' OR AD_TIPOCOBRANCA IS NULL


-- campo valor liquido no financeiro
(
(NVL(FIN.VLRDESDOB,0) +
...
-- informacao de parceiro para multlist
SELECT DISTINCT
CAB.CODEMP AS VALUE,
